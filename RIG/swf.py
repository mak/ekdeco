...
class xSWF(SWF):
    ...
    def parse(self,data):
        import io
        self._data = data = data if isinstance(data, SWFStream) else self.SS(data)
...
class RIG(xSWF):
    ...
    def enc_strings(self):
        ...
        if not hasattr(self,'_enc_strings'):
            ...
            for i,_s in enumerate(self.read_blob(self.binary_data[1].data)):
                s.append(RC4.new(self.rc4keys[i%len(self.rc4keys)]).decrypt(_s))
            self._enc_strings = s
    ...
    def get_clean_shellcode(self):
        ...
        hdr = "\x90" * 0x19
        sh  = ''.join(chr(ord(self._shellcode[0x19+i]) ^ xorb) for i in range(size))
    ...
    def config(self):
        ...
        if not hasattr(self,'_config'):
            ...
            if len(cfg) == 1:
                ...
                self._config = None
                self._shellcode = RC4.new(self._cfgkeys[0]).decrypt(cfg[0]).decode('hex')
                self.get_clean_shellcode()
            else:
                c = []
                for i,x in enumerate(cfg):
                    c.append(unpad(AES.new(self._cfgkeys[i%len(self._cfgkeys)]).decrypt(x)))
                self._config = c
        return self._config
...
if __name__ == '__main__':
    ...
    if x != y:
        print '[-] cant do it';sys.exit(-1)
        
    emb1='CWS'+''.join(map(lambda x: chr(ord(x)^y),data[3:]))
    with open('a.swf','w') as f:f.write(emb1)
    rig = xSWF(StringIO(emb1))
    data = rig.binary_data[31].data.decode('zlib')
    key=data[-16:]
    data = ''.join(( chr( ord(c) ^ ord(key[i%16])) for i,c in enumerate(data[:-16].decode('base64'))))
    with open('b.swf','w') as f: f.write(data)
    ...
    if rig.config:
        print rig.config
        with open('s.bin','w') as f: f.write(rig._shellcode)
        with open('sc.bin','w') as f: f.write(rig.shellcode)
