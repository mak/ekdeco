...
apr.add_argument(
    '-o', '--out', help="prefix for filename used to store decoded chunks")
args = None
...
def decode_fdata(data):
    alphabet = ('za1sd0123456789abcdefghijklmnopqrstuvwxyz'
                'ABCDEFGHIJKLMNOPQRSTUVWXYZ'[5:])
    mixtbl = ('za1sdLMNOPWXY3defghiQRSTUVjklmnABC012'
              'DEFopq456789abcrstuvwxyzGHIJKZ'[5:])
    r = ''.join([alphabet[mixtbl.index(c)]
                 for c in data]).decode('hex').strip('\x00')
    x = ord(r[1]) ^ ord(';')
...
def do_rigv(d):
    ...
    for s in re.findall('<script>(.+?)</script>', d, re.S):
        ...
        for v, _, e, _ in vars:
            x.append(v.split(e))
        payload = ''.join(
            a + b for a, b in zip(x[1], itertools.cycle(x[0][::-1])))
